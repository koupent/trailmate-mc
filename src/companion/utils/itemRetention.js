 ...
 * @returns {Array<{ slot: number, type: number, count: number, name: string, attackDamage: number }>}
 */
export function listOccupiedStacks(bot) {
    const slots = bot?.inventory?.slots || [];
    /** @type {Array<{ slot: number, type: number, count: number, name: string, attackDamage: number }>} */
    const stacks = [];
