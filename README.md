...
| `nearby_loot.owner_clearance` | オーナー近傍で拾わない半径（採掘の邪魔防止） | `3.5` |
| `nearby_loot.give_suppress_ms` | 全回収後に再拾いしない時間 | `12000` |
...
6. `nearby_loot` は半径内にドロップがある間拾い続け、しばらく無くなったら終了します（上限 `max_ms`）。オーナー近傍（`owner_clearance`）のドロップは拾いませんが、死亡復帰・墓回収中は例外です。全回収直後は `give_suppress_ms` の間拾いません。ドロップ回収は墓破壊より優先し、散らばったアイテムの取りこぼしを減らします。
