...
`item_share` の `keep_*` は「残す数」であると同時に「不足を要求する目標数」でもあります。
...
8. `item_share` は、追従中のロック済みオーナーがBotの前方にチェストを置いたとき、そのチェストへ余剰を収納します。防具の各部位・盾・近接武器は装備中を含めて合計2個、食料と松明は合計2スタックを残し、弓・クロスボウ・矢を含むそれ以外を収納します。チャットの「全回収」は従来どおり全アイテムをプレイヤーへ渡します。収納中は開いているチェストの中身を毎回読み直して計画し直すため、途中で拾ったアイテムも対象になります。戦闘や被弾で中断された場合はチェストを閉じて位置を覚え、所有権が戻ってから続きを入れます（`resume_*`）。防具スロットやオフハンドに保持対象外のアイテム（バケツやチェストなど）が入っている場合は、コンテナのスロット範囲外で投入できないため、いったん装備解除してインベントリへ戻してから収納します。チェストが満杯のときや入りきらなかったぶんは手元に残し、結果をチャットで知らせます。
9. `keep_*` の目標数を下回った装備・消耗品は、チャットで補充を求めます（例: `盾、近接武器、食料が足りないよ。持ってきて`）。判定は装備中とインベントリの合計で、食料と松明は残量にかかわらず占有スタック数で数えます。同じ不足が続く間は繰り返さず、不足が悪化したときと、目標数まで補充されてから再び不足したときに言い直します。「全回収」で生じた不足と死亡回収中は要求しません。
