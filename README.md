...
- 耕地の上では跳ばず、ジャンプや落下で耕地へ着地しない（畑を土へ戻さない。平らな畑は歩いて通り、種植えも続けます）
- 経路上の木の扉・フェンスゲートは、開けて通過し、通過後に閉める（鉄の扉・トラップドアは操作しない）
...
主な項目はダッシュボード（http://127.0.0.1:8787）からも編集できます。「設定」タブの
**相棒が手元に残すもの** では、チェスト返却のときに相棒が残すアイテムを、
頭防具・胴防具・脚防具・足防具・盾・近接武器・食料・松明の8タブから1つずつ選べます
（`item_share.retention`。保存してもデスポーンは不要で、ViaProxy も再起動しません）。
...
| `item_share.retention` | 手元に残すアイテムと個数（カテゴリ別）。ダッシュボードの「相棒が手元に残すもの」から編集します | 各カテゴリ `2` |
| `item_share.resume_enabled` | 収納が中断されたとき、所有権が戻ってから続きを入れる | `true` |
| `item_share.resume_expire_ms` | 中断したチェストへ戻るのをあきらめるまでの時間 | `120000` |
| `item_share.resume_retry_ms` | 再開を試す間隔 | `3000` |
| `item_share.resume_max_attempts` | 1つのチェストに対する再開の試行回数上限 | `5` |
| `item_share.max_open_passes` | 1回の設置につきチェストを開き直す上限回数 | `4` |
| `item_share.notify_enabled` | 収納の結果（完了・入りきらない・満杯・あとで続き）を発話する | `true` |
| `item_share.abort_on_recent_damage` | 被弾直後の戦闘ラッチ中も収納を中断する | `true` |

`item_share.retention` は「残す数」であると同時に「不足を要求する目標数」でもあります。形は次のとおりで、カテゴリは
`helmet` / `chestplate` / `leggings` / `boots` / `shield` / `weapon` / `food` / `torch` の8種です。
...
- `limit` … そのカテゴリで残す合計数。装備カテゴリ（防具4部位・盾・近接武器）は最小 `1` で、`1` なら装備中のぶんだけを残します。食料と松明は `0` にできます。
- `items` … アイテムごとの上限。`0` は「残さない」、書かなければ「合計の範囲でいくつでも」。
- 上の例は「近接武器は合計2個。ただし鉄の剣と金の剣を1本ずつで、木の剣は残さない」という意味です。
- 旧版の `keep_torch_stacks` / `keep_food_stacks` / `keep_weapon_stacks` / `keep_equipment_sets` を書いた `config.json` は、`retention` が無いカテゴリの合計としてそのまま読まれます。
...
8. `item_share` は、追従中のロック済みオーナーがBotの前方にチェストを置いたとき、そのチェストへ余剰を収納します。既定では防具の各部位・盾・近接武器は装備中を含めて合計2個、食料と松明・ランタンは合計2スタックを残し、弓・クロスボウ・矢や調理前の食材を含むそれ以外を収納します。何を何個残すかはダッシュボードの「設定」タブから、カテゴリ別のタブでアイテム単位に指定できます（保存すると、デスポーンせずに次の収納から反映されます）。装備中・手持ち中のものは設定にかかわらず収納しません。チャットの「全回収」は従来どおり全アイテムをプレイヤーへ渡します。収納中は開いているチェストの中身を毎回読み直して計画し直すため、途中で拾ったアイテムも対象になります。戦闘や被弾で中断された場合はチェストを閉じて位置を覚え、所有権が戻ってから続きを入れます（`resume_*`）。防具スロットやオフハンドに保持対象外のアイテム（バケツやチェストなど）が入っている場合は、コンテナのスロット範囲外で投入できないため、いったん装備解除してインベントリへ戻してから収納します。チェストが満杯のときや入りきらなかったぶんは手元に残し、結果をチャットで知らせます。
9. `retention` の目標数を下回った装備・消耗品は、チャットで補充を求めます（例: `盾、近接武器、食料が足りないよ。持ってきて`）。判定は装備中とインベントリの合計で、食料と松明は残量にかかわらず占有スタック数で数えます。同じ不足が続く間は繰り返さず、不足が悪化したときと、目標数まで補充されてから再び不足したときに言い直します。「全回収」で生じた不足と死亡回収中は要求しません。
10. 追従オーナーが通常ボートまたは竹のいかだの操縦席へ先に乗り、空席がある場合、相棒は3ブロック以内から2番目の座席へ同乗します。同乗中は相棒から操縦入力を出さず、オーナーの下船・座席順の変化・ボート消失時は相棒も下船して通常追従へ戻ります。チェスト付き・満席のボートには乗りません。
