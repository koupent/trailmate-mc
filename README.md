...
護衛は **Follow / Guard** が主モードです（低体力でも逃走 Retreat には入りません）。プレイヤー近傍（既定 8 ブロック）の敵対モブ、またはボット至近（既定 3.5 ブロック）の敵を護衛対象とし、視界（FOV）は交戦の開始条件にしません。遠すぎる敵や壁越しの新規ロックは避け、一度狙った敵は短い grace で維持します。

- **Follow** … プレイヤーに付く（戦闘しない）
- **Guard** … 護衛対象を優先して倒す／追い払う。複数敵がいるときは、敵の水平角度スパンが狭くなる側へストレイフして囲まれを避ける
...
6. 通常の `nearby_loot` は `awareness_radius` 内のドロップを拾い続けます。Recoveryでは墓破壊前の既存IDを除外し、破壊直後のcapture windowで確定した墓由来IDだけを優先します。無関係なドロップはRecoveryをブロックせず、通常収集へ戻った後に扱います。
