...
6. `nearby_loot` は半径内にドロップがある間拾い続け、しばらく無くなったら終了します（上限 `max_ms`）。通常時は、採掘や墓・チェスト破壊で散らばったアイテムを奪わないよう、オーナー近傍（`owner_clearance`）のドロップを拾いません。死亡復帰・墓回収中は例外です。全回収直後は `give_suppress_ms` の間拾いません。ドロップ回収は墓破壊より優先し、散らばったアイテムの取りこぼしを減らします。
