...
function makeCtx(options = {}) {
    ...
    const bot = {
        ...
        findBlock(query) {
            findCalls++;
            assert.equal(query.maxDistance, 8);
            ...
            return farmlandPositions
                ...
                .sort((a, b) => botPos.distanceTo(a.position) - botPos.distanceTo(b.position))[0] || null;
        },
...
describe('follow crop planting', () => {
    ...
    it('abandons a planting approach when the owner leaves the near radius', async () => {
        const world = makeCtx({ farmland: [new Vec3(6, 63, 0)] });
        const mode = new FollowMode();
    ...
    it('skips occupied or out-of-range farmland and does nothing without a supported item', async () => {
        ...
        const outOfRange = makeCtx({ farmland: [new Vec3(9, 63, 0)] });
    ...
    it('does not plant while the owner is far or a higher-priority action owns the bot', async () => {
        const ownerFar = makeCtx({ ownerPos: new Vec3(13, 64, 0) });
